...
async function getword() {
    ...
    const res = await fetch('http://api.wordnik.com/v4/words.json/randomWords?hasDictionaryDef=true&minCorpusCount=0&minLength=5&maxLength=15&limit=1&api_key=a2a73e7b926c924fad7001ca3111acd55af2ffabf50eb4ae5');
...
let randomeselect = words[Math.floor(Math.random()*words.length)];
console.log(randomeselect);
const correctletters =[];
const wrongletters=[];

function displayword(){
	wordEl.innerHTML = `
    ${Array.from(randomeselect)
			.map(
				letter => `
          <span class="letter">
...
const innertxt = wordEl.innerText.replace(/\s+/g, '');
            

	if (innertxt === randomeselect) {
		finalMessage.innerText = 'Congratulations! You won! game number '+counter;
...
if(e.keyCode >= 65 && e.keyCode <=90){
    ...
    if(randomeselect.includes(keypressed)){
    ...
    ${wrongletters.map(letter=>`
     <span>${letter}</span> `).join('')}
      ...
      if(wrongletters.length  === figureParts.length){
        finalMessage.innerText = 'You Lost ya loser game number ' +counter;
        popup.style.display = 'flex';
    }
...
playAgainBtn.addEventListener('click',()=>{
    ...
    randomeselect = words[Math.floor(Math.random()*words.length)];
    displayword();
    clearman();
    updatewrong();
